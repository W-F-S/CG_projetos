...
@export var xmin: float = -10
@export var xmax: float = 10
@export var step: float = 0.01
@export var scale_xy: float = 50.0
@export var origin: Vector2 = Vector2(320, 240) # centro da tela
@export var show_axes := true
@export var line_width: float = 2.0

@export var a := 1.0 # Raízes e vértice (ponto mínimo)
@export var b := -6.0
@export var c := 5.0
	
func _ready() -> void:
	queue_redraw() # desenhar
...
func _draw_axes() -> void:
	...
	draw_line(origin + Vector2(0, -1000), origin + Vector2(0, 1000), Color(0.2, 0.2, 0.2), 1.0)
...
func _draw_quadratic_features() -> void:
	...
	if delta >= 0.0:
		...
		_draw_point(Vector2(r1, 0.0), Color(1, 0, 0)) ##raizes
		_draw_point(Vector2(r2, 0.0), Color(1, 0, 0))
	var xv := -b / (2.0 * a)
	var yv := f(xv)
	_draw_point(Vector2(xv, yv), Color(0, 0.6, 0))	#ponto minimo
